...
console.log(
  boxen(
    chalk.cyan(figlet.textSync("SEOpilot", { horizontalLayout: "full" })) +
      "\n" +
...
if (urlArg) {
  (async () => {
    const spinner = ora(
      chalk.yellow(`Ejecutando Lighthouse para ${urlArg}...`)
    ).start();
    try {
      ...
      console.log(
        boxen(
          chalk.bold.magenta("🔎 Recomendaciones Lighthouse:") +
            "\n" +
...
function runLocalChecks() {
  ...
  const limit =
    limitIndex !== -1 ? parseInt(process.argv[limitIndex + 1]) : 100;
  ...
  console.log(
    boxen(
      chalk.bold.blueBright(`✈️  SEOpilot: Escaneando carpeta: ${basePath}`),
      {
        padding: 1,
        borderColor: "blue",
        borderStyle: "round",
  ...
  runChecks({
    ...
    if (!isJson) {
      console.log(
        boxen(chalk.green("✅ Análisis completo."), {
          padding: 1,
          borderColor: "green",
          borderStyle: "round",
          align: "center",
