...
export async function runChecks(options: CheckOptions): Promise<boolean> {
  ...
  await pMap(
    ...
    async (file) => {
      ...
      const h1s = $('h1');
      if (h1s.length > 1) errors.push(`Tiene ${h1s.length} <h1>. Solo debe haber uno.`);
      $('img').each((_, el) => {
        if (!$(el).attr('alt')) errors.push(`Imagen sin alt: ${$(el).attr('src')}`);
      });
      if ($('title').length === 0) errors.push(`Falta <title>`);
      if ($('meta[name="description"]').length === 0) errors.push(`Falta <meta name="description">`);
      if (!$('html').attr('lang')) errors.push(`<html> no tiene atributo lang`);
